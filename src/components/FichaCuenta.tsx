...
import { Loader2, X, Calendar, Target, Briefcase, CheckCircle2 } from 'lucide-react';
...
export default function FichaCuenta({ accountId, onClose }: { accountId: string; onClose: () => void }) {
  const { t } = useLanguage();
      ...
      <div style={{
        ...
        {isLoading ? (
          ...
          <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <span style={{ color: 'var(--text-muted)' }}>{t('ficha_error_account')}</span>
          </div>
        ) : (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              {[
                { label: t('target_premium'), value: account.primaObjetivo ? fmt(account.primaObjetivo) : '—', icon: <Target size={16} />, color: '#10b981' },
                { label: t('start_date'), value: fmtDate(account.fechaInicioVigencia), icon: <Calendar size={16} />, color: '#3b82f6' },
                { label: t('ficha_active_cases'), value: String(openCases.length), icon: <Briefcase size={16} />, color: '#f59e0b' },
                { label: t('ficha_closed_cases'), value: String(closedCases.length), icon: <CheckCircle2 size={16} />, color: '#8b5cf6' },
              ].map(kpi => (
